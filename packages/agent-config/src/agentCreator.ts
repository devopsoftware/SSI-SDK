 ...
 * @param filePath
 */
export const getConfig = async (filePath: string): Promise<{ version?: number; [x: string]: any }> => {
  let fileContent: string;

  try {
    fileContent = await FileSystem.readAsStringAsync(FileSystem.documentDirectory + filePath);
  } catch (e) {
  ...
  if (config?.version != 3) {
    const error = new Error('Unsupported configuration file version:' + config.version);
    console.error(error);
    throw error;
  }
